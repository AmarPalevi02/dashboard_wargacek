...
export const useGetLaporan = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<"active" | "validated">("active");
  ...
  const fetchLaporanData = async (page: number = 1, limit: number = 10) => {
    try {
      ...
      const endpoint = filterStatus === "validated" 
        ? `get-all-laporan-validate?page=${page}&limit=${limit}&showDone=true`
        : `get-all-laporan-pending?page=${page}&limit=${limit}`;
  ...
  const changeFilterStatus = (newFilter: "active" | "validated") => {
